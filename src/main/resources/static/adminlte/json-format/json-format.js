window.SINGLE_TAB = "  ";
window.ImgCollapsed = "./js/tools/json-format/Collapsed.gif";
window.ImgExpanded = "./js/tools/json-format/Expanded.gif";
...
function Process(input, output) {
	...
	//var json = " {'type': 'remove_mobile_bind', 'data': { 'session': { 'id':'placeholder_session_id' }}}";
	var json =input;
	var html = "";
	try {
		if (json == "")
		...
		$(output).html("<PRE class='CodeContainer'>" + html + "</PRE>");
		//$("#response").html("<PRE class='CodeContainer'>" + html + "</PRE>");
	} catch (e) {
		alert("JSON数据格式不正确:\n" + e.message);
		$(output).html("");
...
function SetTab(selectSize) {
	window.TAB = MultiplyString(parseInt(selectSize), window.SINGLE_TAB);
}
